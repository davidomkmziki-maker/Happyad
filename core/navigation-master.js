(function(){
  ...
  var MASTER_VERSION='NAV_MASTER_V613_LITE_NO_SHOP';
...
if(page==='profile'||page==='profile_public'){
  ...
  function persistentMainUrl(page){
    ...
    if(page==='message')return 'modules/message-center.html?mode=inbox&source=main-tabs-v594';
  ...
  function preloadFrame(page,url){
    try{
      ...
      try{fr.src=url;}catch(_e){try{fr.setAttribute('src',url);}catch(_x){}}
      return true;
    }catch(_e){return false;}
  }
  function scheduleMainTabsPreloadV594(){
    try{
      if(window[MAIN_TABS_PRELOAD_FLAG])return;window[MAIN_TABS_PRELOAD_FLAG]=true;
      var c=navigator.connection||navigator.mozConnection||navigator.webkitConnection;
      if(c&&c.saveData)return;
      var jobs=[
        {page:'message',delay:220},
        {page:'video',delay:520},
        {page:'profile',delay:980},
        {page:'profile_public',delay:1360},
        {page:'publish',delay:1880}
      ];
      var run=function(){jobs.forEach(function(job){setTimeout(function(){preloadFrame(job.page,persistentMainUrl(job.page));},job.delay);});};
  ...
  function scheduleSoftPreload(){
    try{
      ...
      if(!(c&&c.saveData)){
        ...
        try{prefetchUrl('modules/video.html');}catch(_pv){}
        try{prefetchUrl('modules/message-center.html');}catch(_pm){}
        try{prefetchUrl('modules/publish.html');}catch(_pp){}
      }
  ...
  function minSkeletonMs(page){
    ...
    if(page==='profile')return 1250;
    return 620;
  }
  function maxSkeletonMs(page){
    page=String(page||'');
    if(page==='video')return hasPost(activeUrl)?1150:900;
    if(page==='profile_public')return 900;
    if(page==='profile')return 7200;
    return 4300;
  }
    ...
    if(visitorPersistent){
      ...
      if(!mustReload&&visitorReady){
        try{
          ...
          fr.__happyadVisitorRevealFallbackV601=setTimeout(function(){revealFrame(fr,page,url,'visitor-switch-fallback-v601');},140);
        }catch(_visitorReuse){}
        return true;
      }
    }
  ...
  function close(reason,replace){
    ...
    try{
      ...
      var sh=document.getElementById(TAP_SHIELD_ID);if(sh){sh.classList.remove('on');sh.setAttribute('aria-hidden','true');}
