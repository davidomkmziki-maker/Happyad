(function(){
  'use strict';
  if(window.__HAPPYAD_SHARE_BUTTON_BRIDGE_V567__)return;
  ...
  function domPreview(target){
    try{var root=target.closest('[data-post-id],[data-photo-id],.reel[data-id],.photoSlide,.miniCard,.profilePost');if(!root)return {};var img=root.querySelector('.miniMedia img,.photoMediaBox img,.photoGroupTrack img,.photoSlideMedia img,.profilePostMedia img,img[data-post-media],img[data-publication-media]');var video=root.querySelector('.miniMedia video,.photoMediaBox video,.reel video,video[data-post-media],video[data-publication-media]');var title=root.querySelector('.miniTitle,.profilePostTitle,.slideInfo .title,.info .title,[data-post-title]');var desc=root.querySelector('.miniDesc,.profilePostDesc,.slideInfo .desc,.info .desc,[data-post-description]');var preview=clean((img&&(img.currentSrc||img.src))||(video&&(video.poster||video.getAttribute('poster'))));return {title:clean(title&&title.textContent),description:clean(desc&&desc.textContent),preview_url:preview,thumbnail_url:preview};}catch(_e){return {};}
  }
  function mediaType(target,post){var path=location.pathname.toLowerCase();if(path.indexOf('video.html')>=0||target.closest&&target.closest('.reel[data-id]'))return 'video';var raw=clean(post&&(post.media_type||post.mediaType||post.kind||post.type)).toLowerCase();return /video|reel|clip|mp4|webm|mov/.test(raw)?'video':'photo';}
