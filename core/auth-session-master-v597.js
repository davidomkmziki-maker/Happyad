(function(){
  'use strict';
  if(window.__HAPPYAD_AUTH_SESSION_MASTER_V597__)return;
  window.__HAPPYAD_AUTH_SESSION_MASTER_V597__=true;

  var VERSION='AUTH_SESSION_MASTER_V597';
...
';document.head.appendChild(css);
  ...
  async function requireAuth(intent){
    ...
    /* V597 : popup immédiat et récupération du mot de passe dans un écran dédié. */
    showOverlay(intent);
  ...
  window.HappyAuthSessionV597={version:VERSION,isReady:function(){return ready;},isAuthenticated:isAuthenticated,user:actualUser,refresh:refresh,require:requireAuth,open:open,close:closeOverlay,logout:logout,broadcast:broadcast};
  window.HappyAuthSessionV596=window.HappyAuthSessionV597;
  window.HappyAuthSessionV595=window.HappyAuthSessionV597;
  try{if(window.HappyMasterRegistry)window.HappyMasterRegistry.register('auth-session',{file:'core/auth-session-master-v597.js',responsibility:'session centrale, popup connexion, récupération mot de passe et garde globale',active:true,version:VERSION});}catch(_e){}
})();
