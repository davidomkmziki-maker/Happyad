(function(){
  'use strict';
  if(window.__HAPPYAD_INTERNAL_RETURN_MASTER_V622__)return;
  window.__HAPPYAD_INTERNAL_RETURN_MASTER_V622__=true;

  var VERSION='V622_INTERNAL_RETURN_FIRST_RENDER';
  ...
  function openNotification(detail){
    ...
    /* V622 : ne jamais exposer le calque sombre avant le premier rendu de l'iframe. */
    if(notificationReady)return revealNotification(detail,'already-ready-v622');
    ...
    notificationRevealTimer=setTimeout(function(){
      try{
        ...
        if(d&&d.body&&d.readyState!=='loading')revealNotification(notificationPendingDetail||detail,'safe-document-ready-v622');
      }catch(_e){}
    },2200);
    return true;
  }
  ...
  window.addEventListener('message',function(ev){
    try{
      ...
      if(type==='HAPPYAD_NOTIFICATION_CENTER_READY'){
        notificationReady=true;
        if(notificationPendingDetail)revealNotification(notificationPendingDetail,'message-ready-v622');
      }
