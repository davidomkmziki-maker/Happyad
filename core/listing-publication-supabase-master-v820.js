/* HAPPYAD V820 — publication réelle de toutes les catégories Marketplace.
...
(function(){
  'use strict';
  if(window.__HAPPYAD_LISTING_PUBLICATION_SUPABASE_V820__)return;
  window.__HAPPYAD_LISTING_PUBLICATION_SUPABASE_V820__=true;

  var VERSION='V820_HOME_COVER_VIEWS';
  ...
  async function uploadPublic(c,user,listingId,file,index,payload){
    ...
    await uploadWithRetry(c,PUBLIC_BUCKET,path,file,{upsert:false,cacheControl:'31536000',contentType:file.type||'application/octet-stream'},'Envoi du média '+(index+1),payload);
  ...
  async function uploadPrivate(c,user,listingId,file,kind,index,payload){
    ...
    await uploadWithRetry(c,PRIVATE_BUCKET,path,file,{upsert:false,cacheControl:'3600',contentType:file.type||'application/octet-stream'},'Envoi du justificatif '+(index+1),payload);
    return path;
  }
  ...
  try{if(window.HappyMasterRegistry)window.HappyMasterRegistry.register('listing-publication-supabase',{file:'core/listing-publication-supabase-master-v820.js',responsibility:'publication réelle Marketplace V820 avec médias multiformats, couverture Accueil et vues',active:true,version:VERSION});}catch(_e){}
})();
