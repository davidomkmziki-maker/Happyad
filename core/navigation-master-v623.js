(function(){
  'use strict';
  if(window.__HAPPYAD_NAVIGATION_MASTER_V623__)return;
  window.__HAPPYAD_NAVIGATION_MASTER_V623__=true;

  var MASTER_VERSION='NAV_MASTER_V623_DIRECT_SURFACE_FIRST_PAINT';
  ...
  var SKELETON_STYLE_ID='happyadAppSkeletonStyleV623';
  ...
  function loadFrame(page,url,extra){
    ...
    /* V623 : le clic ouvre immédiatement une surface légère de la destination.
       L'iframe réelle reste invisible seulement jusqu'à son premier paint utilisable. */
  ...
  function acceptFirstRenderV623(ev,data){
    try{
      ...
      fr.setAttribute('data-happyad-first-render-reason-v623',String(data&&data.type||'ready'));
      if(fr.getAttribute('data-happyad-defer-visible')==='1'){
        revealFrame(fr,page,fr.getAttribute('data-happyad-route-url-v601')||fr.getAttribute('data-happyad-src')||pages[page]||'',String(data&&data.type||'first-render-v623'));
      }
      return true;
    }catch(_e){return false;}
  }
  ...
  try{if(window.HappyMasterRegistry)HappyMasterRegistry.register('navigation',{file:'core/navigation-master-v623.js',responsibility:'navigation unique, iframe, retour interne, bouton téléphone, ouverture modules',legacy:['happyadOpenInternalUrlV492','happyadOpenAppPage','happyadCloseAppPage','V492 router','V520 history'],active:true,version:MASTER_VERSION});}catch(_e){}
})();
