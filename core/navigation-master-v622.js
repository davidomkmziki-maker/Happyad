(function(){
  'use strict';
  if(window.__HAPPYAD_NAVIGATION_MASTER_V622__)return;
  window.__HAPPYAD_NAVIGATION_MASTER_V622__=true;

  var MASTER_VERSION='NAV_MASTER_V622_FIRST_RENDER_NO_BLACK';
  ...
  var SKELETON_STYLE_ID='happyadAppSkeletonStyleV617';
...
if(page==='profile'||page==='profile_public'){
  ...
  /* V614 : aucun document lourd n'est téléchargé ou exécuté avant le premier clic utilisateur. */
  ...
  function preloadFrame(page,url){
    try{window.__HAPPYAD_LAZY_FRAME_SKIPPED_V614__={page:String(page||''),url:rootUrl(url||persistentMainUrl(page)),at:Date.now()};}catch(_e){}
    return false;
  }
  function scheduleMainTabsPreloadV594(){
    try{window[MAIN_TABS_PRELOAD_FLAG]=true;window.__HAPPYAD_MAIN_TABS_LAZY_V614__=true;}catch(_e){}
    return false;
  }
  ...
  function maxSkeletonMs(page){
    page=String(page||'');
    if(page==='profile_public')return 1200;
    if(page==='video')return 2600;
    if(page==='profile')return 3200;
    if(page==='message')return 3200;
    return 2600;
  }
  function frameLooksReady(fr,page){
    try{
      if(fr&&fr.getAttribute('data-happyad-first-render-ready-v622')==='1')return true;
  ...
  function ensureFrame(page,url){
    ...
    if(!fr){
      ...
      fr.addEventListener('load',function(){
        ...
        else {showLoader(false);try{fr.removeAttribute('data-happyad-loading');fr.setAttribute('data-happyad-ready-v594','1');fr.style.opacity='';fr.style.visibility='';}catch(_e){}}
      });
      root.appendChild(fr);
    }
  ...
  function visitorFrameReusableV601(fr){
    try{
      ...
      return href!=='about:blank'&&!!d&&d.readyState!=='loading';
    }catch(_e){return !!(fr&&fr.getAttribute('data-happyad-ready-v594')==='1');}
  }
  ...
  function resetVisitorFrameForUrl(url){
    /* V601: le profil visiteur est permanent. Cette ancienne fonction ne détruit plus l'iframe. */
    try{
      var fr=document.getElementById(frameId('profile_public'));
      if(fr)fr.setAttribute('data-happyad-route-url-v601',rootUrl(url||''));
    }catch(_e){}
  }
  ...
  function loadFrame(page,url,extra){
    ...
    var mustReload=visitorPersistent?!visitorHasSource:!sameFrameUrl(fr,url);
    var directMedia=isDirectMediaPage(page);
    /* V622 : toute nouvelle iframe reste invisible jusqu'à son premier rendu.
       La page déjà affichée reste en place; seul le Profil visiteur conserve son squelette dédié. */
    var deferVisible=visitorPersistent?(mustReload||!visitorReady):!!mustReload;
    ...
    if(deferVisible){
      if(visitorPersistent){showVideoDirect(url,false);showSkeleton(page,url,true);}
      else {showVideoDirect(url,false);showSkeleton(page,url,false);}
      try{
        ...
        fr.removeAttribute('data-happyad-first-render-ready-v622');
      }catch(_d){}
    }else{
  ...
  function frameFromMessageSourceV622(source){
    try{
  ...
  function acceptFirstRenderV622(ev,data){
    try{
      var fr=frameFromMessageSourceV622(ev&&ev.source);if(!fr)return false;
      var page=fr.getAttribute('data-happyad-page')||'';
      fr.setAttribute('data-happyad-first-render-ready-v622','1');
      fr.setAttribute('data-happyad-first-render-reason-v622',String(data&&data.type||'ready'));
      if(fr.getAttribute('data-happyad-defer-visible')==='1'){
        revealFrame(fr,page,fr.getAttribute('data-happyad-route-url-v601')||fr.getAttribute('data-happyad-src')||pages[page]||'',String(data&&data.type||'first-render-v622'));
      }
      return true;
    }catch(_e){return false;}
  }
  ...
  window.HappyNavigation={
    ...
    activeFrame:activeFrame,isBusy:navBusy,releaseNavGate:releaseNavGate,pauseFrame:pauseFrame,resumeFrame:resumeFrame,clearVideoRouteMemory:clearVideoRouteMemory,blankVideoFrame:blankVideoFrame,prefetchUrl:prefetchUrl,scheduleSoftPreload:scheduleSoftPreload,
    preloadFrame:preloadFrame,preloadMainTabs:scheduleMainTabsPreloadV594,activateMainTab:activateMainTabV594,openVideoPost:openVideoPostV594,postToFrame:postToFrameV594,deliverVisitorProfile:deliverVisitorTargetV601
  };
  ...
  try{window.addEventListener('message',function(ev){
    try{
      ...
      if(readyType==='HAPPYAD_FIRST_RENDER_READY_V622'||readyType==='HAPPYAD_MESSAGE_CENTER_READY'||readyType==='HAPPYAD_PROFILE_VISUAL_READY_V621C'||readyType==='HAPPYAD_VIDEO_TAB_READY_V594'||readyType==='HAPPYAD_VIDEO_READY'||readyType==='HAPPYAD_PROFILE_READY'){
        acceptFirstRenderV622(ev,d);
      }
  ...
  function bootV16ZJ(){
    ...
    try{window.__HAPPYAD_LAZY_MODULE_BOOT_V614__={at:Date.now(),preloadedFrames:0};}catch(_e){}
  }
  if(document.readyState==='loading')document.addEventListener('DOMContentLoaded',bootV16ZJ,{once:true});else bootV16ZJ();
  try{if(window.HappyMasterRegistry)HappyMasterRegistry.register('navigation',{file:'core/navigation-master-v622.js',responsibility:'navigation unique, iframe, retour interne, bouton téléphone, ouverture modules',legacy:['happyadOpenInternalUrlV492','happyadOpenAppPage','happyadCloseAppPage','V492 router','V520 history'],active:true,version:MASTER_VERSION});}catch(_e){}
})();
