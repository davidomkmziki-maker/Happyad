...
function profileAvatar(profile: Record<string, unknown>): string {
  const value = clean(profile.avatar_url || profile.avatar || profile.photo_url || profile.photo || profile.profile_photo || profile.profile_image_url)
  return /^https:\/\//i.test(value) && value.length <= 2048 ? value : ''
}
...
Deno.serve(async (req) => {
  ...
  async function sendToRows(rows: Array<Record<string, unknown>>, payload: string) {
    let sent = 0
    let failed = 0
    for (const row of rows) {
      try {
        ...
        if (statusCode === 403 || statusCode === 404 || statusCode === 410) {
          await disableEndpoint(clean(row.endpoint))
        }
  ...
  if (action === 'test') {
    ...
    const task = async () => {
      ...
      return sendToRows(subscriptions || [], JSON.stringify({
        ...
        url: './index.html?source=push-test', timestamp: Date.now(),
      }))
    }
    if (delaySeconds > 0) {
  ...
  for (const recipientId of recipients) {
    ...
    const payload = JSON.stringify({
      ...
      title: senderName,
      body: preview,
      // Android/PWA affiche cette image à droite de la notification.
      // Si le profil n'a pas de photo HTTPS, conserver le logo HAPPYAD.
    ...
    const result = await sendToRows(deviceRows, payload)
    sent += result.sent
