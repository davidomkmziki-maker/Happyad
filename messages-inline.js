    ...
    (function(){
      ...
      const HAPPYAD_MSG_CACHE_VERSION = 'V638';
      ...
      function profileFromUrl(){
        ...
        const directId=cleanMsg(happyadDirectUserId);
        const savedId=cleanMsg(saved.auth_uid||saved.auth_id||saved.user_id||saved.uid||saved.uuid||saved.id);
        if(directId && savedId && savedId!==directId) saved={};
        const cached=getCachedMsgProfile(directId)||{};
      ...
      function happyadMsgDirectCandidateList(raw,p){
        ...
        const urlTarget=cleanMsg(raw||q.get('to')||q.get('uid')||q.get('user_id')||q.get('auth_id')||q.get('auth_uid'));
        const savedId=cleanMsg(saved.auth_uid||saved.auth_id||saved.user_id||saved.uid||saved.uuid||saved.id);
        if(urlTarget && savedId && savedId!==urlTarget) saved={};
        return [
          urlTarget,q.get('auth_uid'),q.get('auth_id'),q.get('user_id'),q.get('uid'),q.get('to'),raw,
