/* HAPPYAD V782 — Push : livraison stable, avatar expéditeur vérifié et contrôle volontaire */
'use strict';

var HAPPYAD_SW_VERSION = 'happyad-pwa-V782-push-avatar-local-cache-20260726-1';
var HAPPYAD_STATIC_CACHE = HAPPYAD_SW_VERSION + '-static';
var HAPPYAD_RUNTIME_CACHE = HAPPYAD_SW_VERSION + '-runtime';
var HAPPYAD_MEDIA_CACHE = 'happyad-message-media-v1';
var HAPPYAD_PUSH_STATE_CACHE = 'happyad-push-state-v1';
var HAPPYAD_PUSH_AVATAR_CACHE = 'happyad-push-avatar-v782';
...
var HAPPYAD_APP_SHELL = [
  './',
  './index.html?v=782-push-avatar-local-cache-shell',
  ...
  './core/push-master.js?v=push-v46-avatar-local-cache',
...
function happyadAvatarCacheRequest(detail){
  var sender=String(detail&&detail.sender_id||'sender').replace(/[^A-Za-z0-9_-]/g,'').slice(0,64)||'sender';
  var push=String(detail&&detail.push_id||Date.now()).replace(/[^A-Za-z0-9_-]/g,'').slice(0,72)||String(Date.now());
  return new Request(new URL('./__happyad_push_avatar__/'+sender+'-'+push+'.img',self.location.href).href,{method:'GET'});
}

function happyadPrepareAvatarIcon(data,detail){
  if(!detail||detail.type!=='happyad_message')return Promise.resolve('');
  var remote=happyadNotificationAsset(detail.sender_avatar||data.sender_avatar,'');
  if(!remote)return Promise.resolve('');
  var localRequest=happyadAvatarCacheRequest(detail);
  return fetch(remote,{method:'GET',cache:'no-store',credentials:'omit',redirect:'follow'}).then(function(response){
    if(!response || (!response.ok && response.type!=='opaque'))return '';
    var contentType='';try{contentType=String(response.headers.get('content-type')||'').toLowerCase();}catch(_e){}
    if(response.type!=='opaque' && contentType && contentType.indexOf('image/')!==0)return '';
    return caches.open(HAPPYAD_PUSH_AVATAR_CACHE).then(function(cache){
      return cache.put(localRequest,response.clone()).then(function(){
        return cache.keys().then(function(keys){
          var excess=Math.max(0,(keys||[]).length-40);
          if(!excess)return undefined;
          return Promise.all(keys.slice(0,excess).map(function(key){return cache.delete(key);}));
        });
      }).then(function(){return localRequest.url;});
    });
  }).catch(function(){return '';});
}

function happyadNotificationOptions(data,detail,preparedAvatar){
  ...
  var exactAvatar=String(preparedAvatar||'');
  var icon=exactAvatar||logo;
  var options={
...
function happyadShowNotification(data,detail){
  ...
  return happyadPrepareAvatarIcon(data,detail).then(function(preparedAvatar){
    var options=happyadNotificationOptions(data,detail,preparedAvatar);
...
self.addEventListener('fetch', function(event){
  ...
  if(/\/__happyad_push_avatar__\//i.test(path)){
    event.respondWith(caches.open(HAPPYAD_PUSH_AVATAR_CACHE).then(function(cache){
      return cache.match(request).then(function(cached){
        return cached||new Response('',{status:404,headers:{'Cache-Control':'no-store'}});
      });
    }));
    return;
  }
