...
/* V43 : côté vendeur, bouton Profil client + nom/téléphone/zone affichés */
(function(){
  window.happyadClientAccount = window.happyadClientAccount || {id:'client-demo', name:'Client HAPPYAD'};
  ...
  window.openClientProfile = function(orderId){
    const o=(window.boutiqueOrders||[]).find(x=>String(x.id)===String(orderId));
    if(!o){toast&&toast('Commande introuvable');return;}
    ensureBuyerFields(o);
    const overlay=document.getElementById('paymentOverlay');
    const sheet=overlay?.querySelector('.paymentSheet');
    if(!overlay||!sheet){toast&&toast('Profil client indisponible');return;}
    const name=clientName(o), phone=clientPhone(o)||'Non renseigné';
    const initial=(name.trim()[0]||'C').toUpperCase();
    sheet.classList.add('clientProfileSheet');
    sheet.innerHTML=`
      <div class="sheetTop"><div><b>Profil client</b><span>Compte HAPPYAD lié à la commande</span></div><button class="circleBtn" onclick="closePaymentPopup()">×</button></div>
      ...
      <div class="clientInfoList">
        ...
        <div class="clientInfoItem"><span>Zone choisie</span><b>${o.deliveryZone||'Non renseignée'}${o.deliveryDelayHours?' · '+o.deliveryDelayHours+'h':''}</b></div>
        <div class="clientInfoItem"><span>Réception prévue</span><b>${o.deliveryEtaAt?fmtDateTime(o.deliveryEtaAt):'À confirmer'}</b></div>
  ...
  window.payAndCreateOrder=function(){
    ...
    (window.boutiqueOrders||[]).forEach(ensureBuyerFields);
  };
  window.renderSellerOrders=function(){
    ...
    const list=(window.boutiqueOrders||[]);
    if(!list.length){box.innerHTML=`<div class="sellerOrderCard"><div class="sellerOrderTop"><div class="statusIcon">${typeof icon==='function'?icon('shield','ico'):''}</div><div><b>Aucune livraison</b></div></div></div>`;try{hydrateIcons(box)}catch(e){};return;}
  ...
  try{(window.boutiqueOrders||[]).forEach(ensureBuyerFields);window.renderSellerOrders()}catch(e){console.warn('v43 seller client profile',e)}
})();
