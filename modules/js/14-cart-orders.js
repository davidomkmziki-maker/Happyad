...
(function(){
  ...
  window.renderSellerOrders = function(){
    ...
    box.innerHTML=list.map(o=>{
      ...
      const cancelBox=cancel?`<div class="sellerCancelledBox">Annulation demandée par le client</div>`:'';
      const normalActions=(!completed&&!cancel)?`<button class="btn ${waiting?'dark':''}" style="width:100%;margin-top:10px" onclick="sellerValidateDelivery('${o.id}')" ${waiting?'disabled':''}>${waiting?'Preuve envoyée':'Valider livraison'}</button>`:'';
