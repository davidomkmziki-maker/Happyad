...
(function(){
  ...
  function activeBuyerOrder(o){
    ...
    let actions='';
    if(canClaim){
      if(!o.sellerDelivered && !reached(o)) actions=`<div class="lockedDelivery">Annulation indisponible jusqu’à ${fmt(o.deliveryEtaAt)}</div>`;
      else actions=``;
    }
  ...
  function activeSellerOrder(o){
    ...
    const buttons=cancel?'':`<div class="orderButtons"><button class="btn cancel" onclick="openClientProfile('${h(o.id)}')">Ouvrir profil client</button></div><button class="btn ${waiting?'dark':''}" style="width:100%;margin-top:10px" onclick="sellerValidateDelivery('${h(o.id)}')" ${waiting?'disabled':''}>${waiting?'Preuve envoyée':'Valider livraison'}</button>`;
    const info=waiting?`<div class="adminNotice"><span data-ico="message"></span><span>Preuve envoyée au client.</span></div>`:'';
