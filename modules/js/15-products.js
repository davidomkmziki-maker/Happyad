...
(function(){
  ...
  window.renderCart=function(){
    ...
    const ordersHtml=paidOrders.length?`<div class="orderCard"><h3>Mes commandes</h3>${paidOrders.map(o=>{
      ...
      let actions='';
      if(!done && !cancel){
        if(!o.sellerDelivered && !reached){ actions=`<div class="lockedDelivery">Annulation indisponible jusquՈ ${fmtTime(o.deliveryEtaAt)}</div><button class="btn dark" style="width:100%" onclick="show('messages')">Message vendeur</button>`; }
