...
function renderCart(){
  ...
  const ordersHtml=paidOrders.length?`<div class="orderCard"><h3>Mes commandes</h3>${paidOrders.map(o=>{
    ...
    const claimBtns = !o.clientReceived ? `<div class="claimVisibleRow forceClaimRow"><button class="claimMainBtn" onclick="openResolutionChat('${o.id}','client','Je n’ai pas reçu')">Je n’ai pas reçu</button><button class="claimSecondBtn" onclick="openResolutionChat('${o.id}','client','Revendication')">Revendication</button></div>` : '';
