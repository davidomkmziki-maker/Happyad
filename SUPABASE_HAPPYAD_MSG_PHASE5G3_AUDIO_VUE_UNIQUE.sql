-- ============================================================================
-- HAPPYAD MESSAGES V37 — PHASE 5G3
-- VOCAL / AUDIO EN VUE UNIQUE : CONSOMMATION AU PREMIER DÉMARRAGE RÉEL
-- ============================================================================
-- À exécuter après les SQL 5G1, 5G1B, 5G1C et 5G2 déjà validés.
-- Ce script étend le système maître existant aux messages audio.
-- Il ne crée aucune table et ne modifie aucun autre module HAPPYAD.
-- ============================================================================
...
select pg_advisory_xact_lock(hashtext('happyad_messages_phase5g3_audio_view_once'));

-- --------------------------------------------------------------------------
-- 1. États vue unique : photo + vidéo + audio.
-- --------------------------------------------------------------------------
...
begin
  ...
  where m.conversation_id = p_conversation_id
    ...
    and m.kind in ('audio','photo','video')
    and not exists (
      select 1
...
-- --------------------------------------------------------------------------
-- 2. Réserver une première ouverture audio, photo ou vidéo.
-- Le clic ne consomme rien. Pour l’audio, la consommation est effectuée par
-- le frontend uniquement lorsque la lecture commence réellement.
-- --------------------------------------------------------------------------
...
begin
  ...
  if not found
     ...
     or v_message.kind not in ('audio','photo','video')
  ...
  v_expires := now() + case
    when v_message.kind in ('audio','video') then interval '5 minutes'
  ...
  if found then
    ...
    -- opened_at reste immuable : seule la réservation temporaire change.
  ...
  if not found
     ...
     or (v_message.kind = 'audio' and v_mime not like 'audio/%')
...
-- --------------------------------------------------------------------------
-- 3. Consommation générique après le premier affichage/lecture réel.
-- La photo, la vidéo et l’audio utilisent le même jeton temporaire.
-- --------------------------------------------------------------------------
create or replace function public.happyad_msg_consume_view_once_internal(
  p_message_id uuid,
  p_claim_token uuid
)
returns table (
...
begin
  ...
  if v_consumed_at is null then
    raise exception 'Autorisation expirée ou média déjà ouvert.' using errcode = '42501';
  end if;
...
-- --------------------------------------------------------------------------
-- 4. Nettoyage volontaire chez l’expéditeur : audio + photo + vidéo.
-- --------------------------------------------------------------------------
...
-- --------------------------------------------------------------------------
-- 5. Storage privé : une réservation active autorise l’objet audio/photo/vidéo.
-- Après consommation, aucune nouvelle autorisation Storage n’est accordée.
-- --------------------------------------------------------------------------
...
-- --------------------------------------------------------------------------
-- 6. Aperçu = dernier message encore visible pour le compte connecté.
-- Les métadonnées servent uniquement à distinguer « Vocal » de « Audio ».
-- --------------------------------------------------------------------------
...
begin
  ...
  join lateral (
    ...
    where m.conversation_id = cm.conversation_id
      ...
      and (
        m.view_once is false
        or m.kind not in ('audio','photo','video')
...
select
  'PHASE_5G3_AUDIO_VIEW_ONCE_OK' as status,
...
select count(*)::bigint as view_once_audio_objects_missing
from public.happyad_msg_attachments a
join public.happyad_msg_messages m on m.id = a.message_id
